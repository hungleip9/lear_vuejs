<template>
  <div class="container">
    <div>
    
      <input :style="{border: divBg}" v-model="keyword" type="text" class="search" @focus="onfocus" @keydown.enter="searchName()" placeholder="Tìm kiếm theo tên sản phẩm">
      <img class="imgSearch" src="data:image/png;base64,">
      
      <button class="clear" v-if="keyword.length>=1" @click="clear">X</button>
      <br><br></div>

        <span>
          <div v-if="carts.length > 0 && show == false">
          <table>
            <tr style="height: 20px;">
              <th style="width:70px">STT</th>
              <th>Sản phẩm</th>
              <th>Giá</th>
              <th>Số lượng</th>
              <th>Trạng thái</th>
              
            </tr>
            
            <tr v-for="(cart,index) in carts" :key="index" style="height: 30px;">
              <td>{{++index}}</td>
              <td class="productName" v-if="cart.image.length!=0"><img :src="cart.image"> {{ cart.name }}</td>
              <td class="productName" v-else><img src="https://redzonekickboxing.com/wp-content/uploads/2017/04/default-image.jpg"> {{ cart.name }}</td>
              <td>{{ cart.price.toLocaleString('it-IT', {style : 'currency', currency : 'VND'}) }}</td>
              <td>{{ cart.quantity }}</td>
              <td v-if="cart.isAvailable == true" style="color: #86d383;">Còn hàng</td> 
              <td v-else style="color: #ef646f">Hết hàng</td>          
            </tr>
          </table>
          </div>
          <div v-if="show">
          <table>
            <tr>
              <th style="width:70px">STT</th>
              <th>Sản phẩm</th>
              <th style="width:100px;">Giá</th>
              <th>Số lượng</th>
              <th>Trạng thái</th>
              
            </tr>
            <tr v-for="(cart,index) in products" :key="index">
              <td>{{++index}}</td>
              <td class="productName" v-if="cart.image.length!=0"><img :src="cart.image"> {{ cart.name }}</td>
              <td class="productName" v-else><img src="https://redzonekickboxing.com/wp-content/uploads/2017/04/default-image.jpg"> {{ cart.name }}</td>
              <td>{{ cart.price.toLocaleString('it-IT', {style : 'currency', currency : 'VND'}) }}</td>
              <td>{{ cart.quantity }}</td>
              <td v-if="cart.isAvailable == true" style="color: #86d383;">Còn hàng</td> 
              <td v-else style="color: #ef646f">Hết hàng</td>      
            </tr>
            
          </table>
          </div>
        
        <div v-else-if="carts.length == 0 && keyword.length > 0">
          <table>
            <tr class="dataZezo">
              <th>STT</th>
              <th>Sản phẩm</th>
              <th>Giá</th>
              <th>Số lượng</th>
              <th>Trạng thái</th> 
            </tr>
            <tr>
              <td colspan="5">Không có dữ liệu</td>
            </tr>
          </table>
          
        </div>
        </span>
  </div>

</template>

<script>

export default {

  data (){
    
    return{
        show:true,
        keyword:'',
        message:'',
        divBg:'',
        products:[
          {
            name: 'iPhone 12 Pro Max Chính Hãng',
            image: 'https://cdn.cellphones.com.vn/media/catalog/product/cache/1/image/9df78eab33525d08d6e5fb8d27136e95/p/h/photo_2020-10-13_22-12-24.jpg_1_2.png',
            price: 32990000,
            quantity: 566,
            isAvailable: true
          },
          {
            name: 'iPhone 12 Chính Hãng (VN/A)',
            image: '',
            price: 21790000,
            quantity: 123,
            isAvailable: true
          },
          {
            name: 'iPhone 11 Chính hãng',
            image: 'https://cdn.cellphones.com.vn/media/catalog/product/cache/1/image/9df78eab33525d08d6e5fb8d27136e95/_/0/_0004_layer_5.jpg',
            price: 16690000,
            quantity: 0,
            isAvailable: false
          },
          {
            name: 'Apple iPhone XR 64GB Chính hãng(VN/A)',
            image: 'https://cdn.cellphones.com.vn/media/catalog/product/cache/1/image/9df78eab33525d08d6e5fb8d27136e95/i/p/iphone-xr_5_.jpg',
            price: 12190000,
            quantity: 1023,
            isAvailable: true
          },
          {
            name: 'iPhone 11 Pro Max Chính hãng(VN/A)',
            image: '',
            price: 26500000,
            quantity: 45,
            isAvailable: true
          }
        ],
        carts: [],
        
    }
  },
  methods: {
      onfocus(){
        this.divBg = '1px solid #76b5ec'   
      },
      
      searchName () {
        this.show = false
        this.carts = []
        this.products.forEach((product) => {
            if(product.name.toLowerCase().includes(this.keyword.toLowerCase())){
              console.log(product)
             this.carts.push(product);
          }
        
        })
      },
      clear(){
        this.keyword = '';
        this.show = true;
      }
  },
  computed: {
      
  },
  watch:{
    }
  }
</script>

<style lang="scss" scoped>
    .container {
      
      .dataZezo{
          th{
            width: 80px;
            height: 50px;
          }
          td{
            height: 300px;
          }
        }
      input{
          position: static;
          width: 310px;
          height: 28px;
          border:1px solid black;
          padding-left: 30px;
          text-align: left;
          margin-left: -357px; 
              
      }
      .imgSearch{
          position: absolute;
          width: 25px;
          height: 20px;
          top: 65px;
          left: 300px;
      }
      .clear{
         position: absolute;
          border: none;
          background: white;
          height: 25px;
          top: 65px;
          left: 610px;
          font-weight: bold;
      }
        
      
      margin: 0 auto;
        table{
          width: 700px;
          height: 300px;
          margin: 0 auto;
          text-align: center;
          border: 1px solid black;
          border-collapse: collapse;
          tr{
            padding:0;
            margin: 0;
            th{
              padding:0;
              border-right: 1px solid black;
              border-bottom: 1px solid black;
              background: #f2f6fe;
            }
            td{
              border-right: 1px solid black;
              padding:0;
              img{
              width: 30px;
              height: 30px;
             }
            
            }
          }
          .productName{
              text-align: left!important;
              color:#57aced;
              padding-left: 10px;
            }
        }
    }
</style>
